...
export default function BGGSearch({ onAddGame }: BGGSearchProps) {
  ...
  const handleSearch = useCallback(
    async (query: string, page: number, isRetry: boolean = false) => {
      ...
      try {
        const response = await axios.get(
          `/api/bgg-search?query=${encodeURIComponent(query)}&page=${page}`,
        );
        if (response.data.games) {
          setSearchResults((prevResults) =>
            page === 1
              ? response.data.games
              : [...prevResults, ...response.data.games],
          );
        ...
        <div className='mt-4 space-y-4'>
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4'>
            {searchResults.map((game: BGGGame) => (
              <div
                key={game.id}
                className='bg-white border-2 border-black rounded-lg p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-1 hover:-translate-y-1 transition-transform duration-200'
              >
                <Image
                  src={game.imageUrl}
                  alt={game.name}
                ...
                <p className='text-sm mb-2'>
                  Rating: {game.averageRating} ({game.numVoters} voters)
                </p>
